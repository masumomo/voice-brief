...
**Gemini 2.0 Flash コスト試算:**

| 項目 | 入力トークン | 出力トークン |
|------|-------------|-------------|
| 価格（100万トークンあたり） | $0.10 | $0.40 |

実運用データ（Weekly: 43イベント取得、25件ブリーフィング生成）での試算:

| 処理 | 入力トークン | 出力トークン | コスト |
|------|-------------|-------------|--------|
| カテゴリ判定（43件） | ~7,000 | ~200 | $0.0008 |
| イベント要約（43件） | ~22,000 | ~4,300 | $0.0039 |
| ブリーフィング生成 | ~3,500 | ~1,500 | $0.0010 |
| **合計（1回）** | **~32,500** | **~6,000** | **~$0.006** |

**月次コスト目安（全機能Gemini Flash使用時）:**

| 実行頻度 | 月間コスト |
|---------|-----------|
| Daily実行（30回/月） | ~$0.18/月 |
| Weekly実行（4回/月） | ~$0.024/月 |
| Daily + Weekly | ~$0.20/月 |

※ 無料枠内で十分運用可能（1日1500リクエストまで無料）
