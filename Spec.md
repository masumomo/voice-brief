...
| **プロジェクト名** | VoiceBrief |
| **バージョン** | v1.0 |
| **目的** | 育児中エンジニアのための完全音声キャッチアップツール |
| **実行環境** | macOS (Local) / Go Runtime |

## 1. システム概要

SlackおよびNotionの更新情報をAPI経由で取得し、重要度ベースでフィルタリング・要約した後、macOS標準のTTS（Text-to-Speech）機能を用いて音声ファイル（.m4a）を生成するCLIツール。`launchd`による定期実行を前提とし、ユーザーは生成された音声ファイルを聞くだけで直近の業務状況を把握できる。
